...
std::pair<float, std::vector<int>> Trajectoy_analysis::perform_DTW(Joint::Type joint, Distances type) {
...
std::pair<float, std::vector<int>> Trajectoy_analysis::perform_DTW(const std::vector<Quaternion*> &inp_traj, const std::vector<Quaternion*> &ref_traj) {
    float *c_matrix = Dtw::dtw(inp_traj, ref_traj, quaternion_dist);
    return Dtw::get_cost_and_alignment(c_matrix, inp_traj.size(), ref_traj.size());
}
