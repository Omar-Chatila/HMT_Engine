...
public:
    static float lcss(const std::vector<Vec3D *> &inpF, const std::vector<Vec3D *> &refF, float epsilon, std::function<float(const Vec3D*, const Vec3D*)> func);
    static float lcss(const std::vector<Quaternion *> &inpF, const std::vector<Quaternion *> &refF, float epsilon, float delta, std::function<float(const Quaternion*, const Quaternion*)> func);
};
