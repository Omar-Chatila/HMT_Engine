...
float Frechet::frechet(const std::vector<Quaternion *> &inpF, const std::vector<Quaternion *> &refF, std::function<float(const Quaternion*, const Quaternion*)> func) {
    size_t n = inpF.size();
    size_t m = refF.size();
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            ...
            if (i > 0 && j > 0)
                C[INDEX(i, j)] = std::max(std::min(C[INDEX(i - 1, j)], std::min(C[INDEX(i - 1, j - 1)], C[INDEX(i, j - 1)])), d);
            else if (i > 0 && j == 0)
